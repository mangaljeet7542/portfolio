    ...
    const handleSubmit = async () => {
        ...
        if (valid) {
            try {
                ...
                await emailjs.send("service_g2nq2f5","template_msa041q", {to_name: 'Mangaljeet', from_name: formData.name, message: formData.message+` Phone No: ${formData.phone} and Email Address: ${formData.email}`}, "ckQCm-azAtVgplBlZ" );
            ...
            toast.error('Some error occurred! Please correct the errors and try again.', { duration: 4000 });
        }
    }
    ...
    return (
        <div className="px-16 md-mx:px-8 sm-mx:px-4 mx-20 lg-mx:mx-10 md-mx:mx-0 my-10 font-mono" id="Contact">
            ...
            <div data-aos="flip-left" data-aos-duration="800" className="w-[70%] lg-mx:w-full shadow-[0_0_10px_0_#64FFDA50] m-auto flex flex-col gap-6 border border-primaryColor p-8 rounded-3xl sm-mx:p-4">
                ...
                <Button fullWidth onClick={handleSubmit} rightSection={<IconArrowRight size={20} />} className="!text-bgColor !font-bold" variant="filled" size={btn} radius="lg" color="#64FFDA">
                    Send
                </Button>
            </div>
        </div>
